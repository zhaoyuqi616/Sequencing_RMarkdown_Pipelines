---
title: "Report of m6A-seq Analysis"
author: | 
 |
...
Mapped reads of IP and input libraries were provided for R package [exomePeak](https://bioconductor.org/packages/exomePeak), which identifies m6A peaks with bed or bigwig format that can be adapted for visualization on the [IGV software](http://www.igv.org).  
[MEME](http://meme-suite.org) and [HOMER](http://homer.ucsd.edu/homer/motif) were used for de novo and known motif finding followed by localization of the motif with respect to peak summit. Called peaks were annotated by intersection with gene architecture using R package [ChIPseeker](https://bioconductor.org/packages/ChIPseeker).  
...
![Basic bioinformatics workflow](/Volumes/YZWD/Projects_COH/m6A_seq/IGC-SM-19367__IGC-SM-19366/m6A_bioinfo.png) 
...
setwd("/Volumes/YZWD/Projects_COH/m6A_seq/IGC-SM-19367__IGC-SM-19366/Processed_Bams/m6A/")
...
Sequencing_samples<-read.table("m6A-seq_samples.txt",sep = "\t",header = TRUE)
...
![General statistics of QC in  fastp](/Volumes/YZWD/Projects_COH/m6A_seq/IGC-SM-19367__IGC-SM-19366/Processed_Bams/m6A/Fastp_General_Statistics.png)  
The RNA-seq data is of high duplication.
...
## Epitranscriptome Analysis by exomePeak  
The **exomePeak** package is developed for the analysis of affinity-based epitranscriptome shortgun sequencing data from MeRIP-seq (maA-seq).  
```{r Epitranscriptome Analysis by exomePeak,fig.height = 8,fig.width = 12,include = TRUE,echo=FALSE,message = FALSE,warning = FALSE,error=TRUE}
library(exomePeak)
BAM_files<-dir("/Volumes/YZWD/Projects_COH/m6A_seq/IGC-SM-19367__IGC-SM-19366/Processed_Bams/",full.names = TRUE,pattern = ".bam$")
mm10.ncbiRefSeq.gtf <-"/Volumes/YZWD/Projects_COH/m6A_seq/IGC-SM-19367__IGC-SM-19366/mm10.ncbiRefSeq.gtf"
# result_WT_vs_Input <- exomepeak(GENE_ANNO_GTF=mm10.ncbiRefSeq.gtf,IP_BAM=BAM_files[1:3],INPUT_BAM=BAM_files[7:9],OUTPUT_DIR = "WT_vs_Input")
# result_KO_vs_Input <- exomepeak(GENE_ANNO_GTF=mm10.ncbiRefSeq.gtf,IP_BAM=BAM_files[4:6],INPUT_BAM=BAM_files[10:12],OUTPUT_DIR = "KO_vs_Input")
# result_KO_vs_WT <- exomepeak(GENE_ANNO_GTF=mm10.ncbiRefSeq.gtf,IP_BAM=BAM_files[1:3],INPUT_BAM=BAM_files[7:9],TREATED_IP_BAM=BAM_files[4:6],TREATED_INPUT_BAM=BAM_files[10:12],OUTPUT_DIR = "exomePeak_KOvsWT")
...
peak<-readPeakFile("/Volumes/YZWD/Projects_COH/m6A_seq/IGC-SM-19367__IGC-SM-19366/Processed_Bams/m6A/exomePeak_KOvsWT/sig_diff_peak.bed")
...
![HOMER Motif Enrichment Summary](/Volumes/YZWD/Projects_COH/m6A_seq/IGC-SM-19367__IGC-SM-19366/Processed_Bams/m6A/Homer_MotifOutput.png)
...
#shiny::includeHTML("/Volumes/YZWD/Projects_COH/m6A_seq/IGC-SM-19367__IGC-SM-19366/Processed_Bams/m6A/Homer_MotifOutput/homerResults.html")
```
...
sig_diff_peak<-read.table("exomePeak_KOvsWT/sig_diff_peak.bed",header = FALSE,sep = "\t")
data("motifAnnotations_mgi")
geneLists <- list(geneListName=unique(sig_diff_peak$V4))
...
col_groups[col_groups=="WT_IP"]="darkblue"
col_groups[col_groups=="KO_IP"]="darkred"
col_groups[col_groups=="WT_input"]="lightblue"
col_groups[col_groups=="KO_input"]="red"
...
pca_transcript<-fviz_pca_ind(PCA_Transcript_Results,habillage = colData$groups,addEllipses=TRUE, repel = TRUE,ellipse.type = "confidence",palette = c("#02d85f","#ab4b52","#7788aa","#ffd728"),title ="PCA by Transcripts")
PCA_Gene_Results<-prcomp(t_normalized_counts_Gene)
#fviz_eig(PCA_Gene_Results)
pca_gene<-fviz_pca_ind(PCA_Gene_Results,habillage = colData$groups,addEllipses=TRUE, repel = TRUE,ellipse.type = "confidence",palette = c("#02d85f","#ab4b52","#7788aa","#ffd728"),title ="PCA by Genes")
...
#Samples_IP<-c("COHP_44188","COHP_44190")
Samples_IP<-c("COHP_44422","COHP_44423","COHP_44424","COHP_44425","COHP_44426","COHP_44427")
pheno_data_filter<-colData %>% filter(rownames(colData) %in% Samples_IP)
pheno_data_filter$groups<-gsub("WT_","Control_",pheno_data_filter$groups)
...
Average_Norm_Counts_Gene<-data.frame(Average_Norm_Counts_Control=log2(rowMeans(normalized_counts_Gene[,rownames(pheno_data_filter)[pheno_data_filter$groups=="Control_IP"]])+1),Average_Norm_Counts_KO=log2(rowMeans(normalized_counts_Gene[,rownames(pheno_data_filter)[pheno_data_filter$groups=="KO_IP"]])+1))
dds_Transcript <- estimateSizeFactors(dds_Transcript)
normalized_counts_Transcript <- counts(dds_Transcript, normalized=TRUE)
Average_Norm_Counts_Transcript<-data.frame(Average_Norm_Counts_Control=log2(rowMeans(normalized_counts_Transcript[,rownames(pheno_data_filter)[pheno_data_filter$groups=="Control_IP"]])+1),Average_Norm_Counts_KO=log2(rowMeans(normalized_counts_Transcript[,rownames(pheno_data_filter)[pheno_data_filter$groups=="KO_IP"]])+1))
...
save.image("m6A_seq_Analysis_Results.RData")
```
